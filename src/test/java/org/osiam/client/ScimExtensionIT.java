...
public class ScimExtensionIT extends AbstractIntegrationTestBase {
    ...
    public void retrieving_a_user_with_extension_works() {
        ...
        for (Map.Entry<String, String> entry : extensionData.entrySet()) {
            assertEquals(entry.getValue(), storedExtension.getField(entry.getKey(), ExtensionFieldType.STRING));
        }
    }
    ...
    private Extension createExtensionWithData(String urn, Map<String, String> extensionData) {
        Extension extension = new Extension(urn);
        
        for (Entry<String, String> fieldData : extensionData.entrySet()) {
            extension.addOrUpdateField(fieldData.getKey(), fieldData.getValue());
        }
